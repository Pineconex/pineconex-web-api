...
create and validate strategies, run backtests / parameter sweeps / robustness tests, and
launch and monitor live trading bots — from your own scripts or an AI agent.

## Getting an API key

Create a **personal access token** in the web app under **Account → API keys**. It looks like
`pcx_live_…` and is shown once. Send it as `Authorization: Bearer <key>` on every request.

```bash
export PINECONEX_API_URL=https://pineconex.com
export PINECONEX_API_KEY=pcx_live_your_key_here
curl -H "Authorization: Bearer $PINECONEX_API_KEY" "$PINECONEX_API_URL/api/v1/strategies"
```

> **Driving PineconeX from Claude or another AI agent?** Install the companion skill —
> [https://github.com/Pineconex/pineconex-web-skill](https://github.com/Pineconex/pineconex-web-skill) — set `PINECONEX_API_KEY` in its environment, and it
> follows the workflows below.

## License

Documentation licensed under [CC BY 4.0](./LICENSE) — © 2026 PineconeX.

---

<!-- The reference below is generated from the PineconeX source of truth. Do not edit by hand;
     run scripts/sync-api-docs.sh in the pineconex-web repo instead. -->
